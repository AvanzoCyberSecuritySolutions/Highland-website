...
class _NewBookingsState extends State<NewBookings> {
  ...
  callFuction() async {
    // This check prevents errors if the widget is removed from the tree before the async operation completes.
    if (mounted) {
      Provider.of<BookingPatientController>(context, listen: false)
          .departmentAndDoctorSelection();
    }
  ...
  Future<void> _selectDate(
    ...
    picked = await showDatePicker(
      context: context,
      initialDate: DateTime.now(),
      firstDate: DateTime.now(), // Users can't book appointments in the past
      lastDate: DateTime(2101),
    );
    if (picked != null) {
      setState(() {
      ...
      // Fetch new timeslots for the selected date
      selectingTimeslots(
          varprovider: varprovider,
          functionprovider: functionprovider,
          date: _dateController.text.trim());
  ...
  selectingTimeslots(
      ...
      required BookingPatientController functionprovider,
      required String date}) async {
    // Ensure a doctor is selected before fetching time slots
    if (_selectedDoctor == null) return;

    int itemid = varprovider.doctorList.indexOf(_selectedDoctor!);
    if (itemid == -1) return; // Doctor not found, do nothing
  ...
  Widget build(BuildContext context) {
    ...
    fieldSubmitted() async {
      ...
      if (varprovider.patientDetailsModel == null ||
          varprovider.patientDetailsModel?.firstName == null) {
        // Clear all fields if no patient is found
        _emailController.clear();
        _reasonController.clear();
        _dateController.clear();
        firstnamecontroller.clear();
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('No patient found with that ID'),
            backgroundColor: ColorConstant.mainRed,
          ),
        );
      } else {
        ...
        // Update department and doctors safely
        setState(() {
          _selectedDepartment = varprovider.patientDetailsModel?.department;
        });
        ...
        child: SingleChildScrollView(
          child: Center(
            child: Container(
              ...
              child: Form(
                key: _formKey,
                // ▼▼▼ WRAPPED YOUR WIDGETS IN A COLUMN ▼▼▼
                child: Column(
                  children: [
                    TextFormField(
                      controller: patientidcontroller,
                      ...
                      decoration: InputDecoration(
                        labelText: 'Patient Id',
                        prefixIcon:
                            Icon(Icons.person, color: Color(0xFF1FBCB1)),
                        filled: true,
                        fillColor: Colors.white,
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(8.0),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (newValue) async {
                        setState(() {
                          ...
                          varprovider.doctorList
                              .clear(); // Clear old doctor list
                          varprovider.timeList.clear();
                          selectedindex = null;
                        });
                        await functionprovider.departmentAndDoctorSelection(
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (newValue) async {
                        setState(() {
                          _selectedDoctor = newValue;
                          varprovider.timeList.clear(); // Clear old time slots
                          selectedindex = null;
                        });
                        // Re-fetch time slots if date is already selected
                        if (_dateController.text.isNotEmpty) {
                          await selectingTimeslots(
                              varprovider: varprovider,
                              functionprovider: functionprovider,
                              date: _dateController.text.trim());
                        }
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Reason (Optional)',
                        prefixIcon: Icon(Icons.edit, color: Color(0xFF1FBCB1)),
                        filled: true,
                        fillColor: Colors.white,
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(8.0),
                    ...
                    TextFormField(
                      ...
                      decoration: InputDecoration(
                        labelText: 'Date',
                        prefixIcon: Icon(Icons.timer, color: Color(0xFF1FBCB1)),
                        filled: true,
                        fillColor: Colors.white,
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(8.0),
                    ...
                    const SizedBox(height: 20.0),
                    // Your time slot logic can be added here
                    if (varprovider.timeList.isNotEmpty)
                      Wrap(
                        spacing: 8.0,
                        runSpacing: 8.0,
                        children:
                            List.generate(varprovider.timeList.length, (index) {
                          final isSelected = selectedindex == index;
                          final isBooked = !varprovider.selectedtimeList
                              .contains(varprovider.timeList[index]);

                          return ChoiceChip(
                            label: Text(varprovider.timeList[index]),
                            selected: isSelected,
                            onSelected: isBooked
                                ? null
                                : (bool selected) {
                    ...
                    Center(
                      child: ElevatedButton(
                        onPressed: () async {
                          if (_formKey.currentState!.validate() &&
                              selectedindex != null) {
                            // ▼▼▼ CORRECTED: Safe null check before accessing time ▼▼▼
                            String? selectedTime;
                            if (selectedindex! <
                                varprovider.doctorTimelistModel.slots!.length) {
                              selectedTime = varprovider.doctorTimelistModel
                                  .slots?[selectedindex!].time;
                            }
                            ...
                            if (varprovider.isSuccessful == true) {
                              showDialog(
                                context: context,
                                builder: (context) {
                                  return AlertDialog(
                                    ...
                                    actions: [
                                      TextButton(
                                        ...
                                        onPressed: () {
                                          ...
                                          phnumbercontroller.clear();
                                          functionprovider
                                              .clearPatientDetails(); // Clear model data
                                          // ▼▼▼ CORRECTED: Use setState to update UI ▼▼▼
                                          setState(() {
                                            varprovider.deptList.clear();
                                            varprovider.doctorList.clear();
                                            varprovider.timeList.clear();
                                          ...
                                          callFuction(); // Re-fetch initial departments
                                          Navigator.pop(context);
                                        },
                                      ),
                                    ],
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: const Text(
                                      'Appointment not Booked. The slot may be taken.'),
                                  backgroundColor: ColorConstant.mainRed,
                                ),
                              );
                            }
                            ...
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                content: const Text(
                                    'Please fill all required fields and select a time.'),
                                backgroundColor: ColorConstant.mainRed,
                              ),
                            );
                          }
